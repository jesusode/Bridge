...
class TkSpreadsheet(Frame):

    def __init__(self, parent, font=None, **keywords):
        Frame.__init__(self, parent)
        self.columns=[]
        #Espacio vertical extra en las celdas-----
        self.extrah=10
        self.colnames=[]
        #-----------------------------------------
    ...
    def addRow(self, pos, row):
        ...
        #Dar un poco mas de espacio----------------------
        row.height=self.getRowHeight(row) + self.extrah
        #------------------------------------------------
    ...
    def getCellValue(self,row,col):
        return self.rows[row].widgets[col].get()

    def setCellValue(self,row,col,value):
        self.rows[row].widgets[col].delete(0,'end') 
        return self.rows[row].widgets[col].insert('end',value)

    def lockCell(self,row,col):
        self.rows[row].widgets[col].configure(state='readonly')
        return 1

    def unlockCell(self,row,col):
        self.rows[row].widgets[col].configure(state='normal')
        return 1      
            
    def configureCell(self,row,col,options):
        self.rows[row].widgets[col].configure(**options)
        return 1

    def configureRow(self,row,options):
        for widget in self.rows[row].widgets:
            widget.configure(**options)
        return 1

    def configureColumn(self,col,options):
        for i in range(len(self.rows)):
            self.rows[i].widgets[col].configure(**options)
        return 1
    #----------------------------------------------------------------------------------------------
...
if __name__ == '__main__':
    tk = Tk()
    ssw = TkSpreadsheet(tk, width=900,font=Font(family='Helvetica',size=9,underline=1,weight='bold',slant='italic',overstrike=1))
    ...
    for i in range(0,10):
        ...
        ssw.addRow(END, ('Important Message: %d' % i, 'John Doe','10/10/%04d' % (1900-i)))
