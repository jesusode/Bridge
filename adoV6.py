      ...
      #try:
        ...
        if not self.__adoRS.EOF:
          ...
          while not self.__adoRS.EOF:
            if type(self.__adoRS.Fields(campo).Value)==types.UnicodeType:              
...
class Recordset:
  ...
  def setCursorLocation(self,loc):
      if adoCursorLocation.has_key(loc):
