...
import tkFont
import Tkinter
import ScrolledText #ScrolledText nativo Tkinter
try:
    import Tix
except:
    HAS_TIX=0
...
_types=['text','textbox','textshow','password','regexp','combo','richtextbox',
        ...
        'colors','spread','table','button','buttonimg','tree','progress','check','canvas','multilist','progress2']
_entry=None
_rw=None
_cl=None
_w=0
...
class TkScrolledCanvas:
    def __init__(self, master, **opts):
        ...
        self.canvas = Tix.Canvas(self.frame, **opts)
        self.canvas.grid(row=0, column=0, sticky="nsew")
        self.vbar = Tix.Scrollbar(self.frame, name="vbar")
        self.vbar.grid(row=0, column=1, sticky="nse")
        self.hbar = Tix.Scrollbar(self.frame, name="hbar", orient="horizontal")
...
def messageBox(*args): #tkMessageBox
    ...
    if len(args)not in [3,4]: 
        ...
        La sintaxis correcta es _messageBox(title,message,type[,icon])'''
        raise Exception(msg)    
    ...
    if icon=='info':
        ...
        retval=tkMessageBox.showwarning(title,msg,type=t)
    elif icon=='error':
        retval=tkMessageBox.showerror(title,msg,type=t)
    elif icon=='question':
        retval=tkMessageBox.askquestion(title,msg,type=t)
...
def getTkColor(*args):
    ...
    if len(args)!=1: 
        msg='''Numero incorrecto de argumentos para la funcion.
        La sintaxis correcta es _getTkColor(title)'''
        raise Exception(msg)
    top=None
    if __TK_APP__==None:
        top=None
        ...
        top.withdraw()
        top.wm_iconbitmap('icono.ico')
    color=tkColorChooser.askcolor(title=args[0])
    if top and top.winfo_children()==[]:
        top.quit()    
    if color!=(None,None):
        r,g,b=color[0]
...
def getTkFile(*args):
    ...
    if len(args)!=2: 
        msg='''Numero incorrecto de argumentos para la funcion.
        La sintaxis correcta es _getTkFile(title,mode)'''
        raise Exception(msg)
    top=None
    if __TK_APP__==None:
        top=None
    ...
    if args[1]=='save':
        file=tkFileDialog.asksaveasfilename(title=args[0])
    else:
        file=tkFileDialog.askopenfilename(title=args[0])
    if top and top.winfo_children()==[]:
        top.quit()            
    if file:
        return file
...
def getTkFiles(*args):
    ...
    if len(args)!=1: 
        msg='''Numero incorrecto de argumentos para la funcion.
        La sintaxis correcta es _getTkFiles(title)'''
        raise Exception(msg)
    top=None
    if __TK_APP__==None:
        top=None
        ...
        top.wm_iconbitmap('icono.ico')        
    files=[]
    files=tkFileDialog.askopenfilename(title=args[0],multiple=True)
    if top and top.winfo_children()==[]:
        top.quit()    
    if files:
...
def getTkDir(*args):
    ...
    if len(args)!=1: 
        msg='''Numero incorrecto de argumentos para la funcion.
        La sintaxis correcta es _getTkDir(title)'''
        raise Exception(msg)
    top=None
    if __TK_APP__==None:
        top=None
        ...
        top.withdraw()
        top.wm_iconbitmap('icono.ico')    
    dir=tkFileDialog.askdirectory(title=args[0])
    if top and top.winfo_children()==[]:
        top.quit()     
    if file:
        return dir
...
def formBox(*args): #
    ...
    if len(args)not in [4,6,8]: 
        msg='''Numero incorrecto de argumentos para la funcion.
        La sintaxis correcta es _formBox(app_options,label_list,label_types_dict,data_dict[,cancellable,noclose,menu_labels,menu_dict])'''
        raise Exception(msg)
    appopts=args[0]
    ...
    bodyframe=ttk.Frame(_toplevelf)
    ...
    #mainframe=None
    if noclose==1:
        mainframe2=ttk.Frame(bodyframe) #(_toplevelf)
    ...
    #Control del estilo--------------------------------
    if not style in  list(ttk.Style().theme_names()):
        style='default'
    ...
    #-------------------------------------------------
    if noclose==0:    
        ok=ttk.Button(bodyframe,text='OK',command=_getFormValues)
        ok.grid(column=0,row=1,sticky='NSEW',padx=5,pady=5)
        if cancellable==1:
            cancel=ttk.Button(bodyframe,text='Cancel',command=_cancelForm)
...
def addFormItem(fname,itemlabel,itemvalues,sizer,nolabels): #Anyade dinamicamente un widget
    ...
    buildWidget(itemlabel.keys()[0],_mainframe,sizer,nolabels,_last_rowcount)
...
def buildWidget(label,mainframe,sizer,nolabels,rowcont):
        ...
        if _labels[label]=='separator':
            widget=ttk.Separator(mainframe)
            if sizer=='pack':
            ...
            if _values.has_key(label):
                ...
                if os.path.exists(_values[label]):
                    ...
                    widget=ttk.Label(mainframe,image=img)
                else:
                    widget=ttk.Label(mainframe,text=_values[label])
            ...
            if _values.has_key(label):
                ...
                widget=ttk.Label(mainframe,image=img,text=txt,compound=comp)
                _widgets[label]=widget
                if sizer=='pack':
                    widget.pack(side='top', expand='yes', fill='both',padx=5,pady=5)
                else:                
        ...
        if nolabels==0:
            lbl=ttk.Label(mainframe,text=label)
            if sizer=='pack':
        ...
        if _labels[label]=='text':
            ...
            widget=Tkinter.Spinbox(mainframe,from_=initval,to=sys.maxint)
            _widgets[label]=widget
        elif _labels[label]=='password':
            widget=ttk.Entry(mainframe,show='*')
            _widgets[label]=widget
            ...
            if HAS_TIX:
                widget=Tix.ScrolledText(mainframe,height=80,scrollbar='both')
                _widgets[label]=widget
                if _values.has_key(label):
                    widget.text.config(wrap=Tkinter.WORD)
                    widget.text.insert('end',_values[label])
            else:
                widget=ScrolledText.ScrolledText(mainframe,height=5)
                _widgets[label]=widget
                if _values.has_key(label):
            ...
            if not HAS_TIX:
                return 0
            widget=Tix.ScrolledText(mainframe,height=80,state='readonly',scrollbar='none')
            _widgets[label]=widget
            if _values.has_key(label):
                widget.text.config(wrap=Tkinter.WORD)
                widget.text.insert('end',_values[label])                
            ...
            if _values.has_key(label):
                ...
                widget=ttk.Combobox(mainframe,values=values,state='normal')
                _widgets[label]=widget
            widget.current(0)
            ...
            if _values.has_key(label):
                if not HAS_TIX:
                    ...
                    values=_values[label]
                    widget=Tix.ScrolledListBox(mainframe,height=60)
                    _widgets[label]=widget
            ...
            widget=ttk.Entry(mainframe, state='readonly')
            _widgets[label]=widget
            ...
            if _values.has_key(label):
                ...
                widget=ttk.Treeview(mainframe,show="tree headings")
                ...
                if not _values.has_key(label):
                    raise Exception('Error: Debe definirse un codigo a ejecutar para el boton "%s"'%label)
                bcode= _values[label]
                btext=label
                if _values.has_key(label + '_text'):
                    btext=_values[label+'_text']                
                widget=ttk.Button(mainframe,text=btext,command=lambda c=bcode:bcode())
                _widgets[label]=widget
                ...
                if _values.has_key(label + '_text'):
                    btext=_values[label+'_text']
                if _values.has_key(label + '_image'):
                    bimage=_values[label+'_image']
                if _values.has_key(label + '_style'):
                    bstyle=_values[label+'_style']
                photo=PILImage.open(bimage)
                img=PILImageTk.PhotoImage(photo)
                images_refs.append(img)                    
                widget=ttk.Button(mainframe,text=btext,image=img,compound=bstyle,command=lambda c=bcode:bcode())
                _widgets[label]=widget                
        elif _labels[label]=='check':
                v=Tkinter.IntVar()
                widget=ttk.Checkbutton(mainframe, variable=v)
                _checkboxes[label]=v
                _widgets[label]=widget          
        elif _labels[label]=='progress':
                length=200
                v=Tkinter.IntVar()
                widget=ttk.Progressbar(mainframe,orient ="horizontal",length = length, mode ="determinate",maximum=100,variable=v)
                if _values.has_key(label):
        ...
        if _values.has_key(label+'_popupmenu'):
            ...
            mlbs=_values[label + '_menulabels']
            mnus=_values[label + '_menus']
            if isinstance(widget,Tix.ScrolledText):
...
##            elif _labels[label]=='canvas':
##                _dictvalues[label]=_widgets[label].postscript()                  
            elif _labels[label]=='font':
...
def _exitForms(*args):
    ...
    if len(args)!=0: 
        msg='''Numero incorrecto de argumentos para la funcion.
        La sintaxis correcta es _exitForms()'''
        raise Exception(msg)      
    #print '_top: %s' %_top
...
#    color=tkColorChooser.askcolor(title='Color')
...
def _showColor(event):
    ...
    if 'win32' in sys.platform:
        _toplevelf.wm_attributes('-disabled',1)
    color=tkColorChooser.askcolor(title='Choose a color')
    if color!=(None,None):
        r,g,b=color[0]
        event.widget.insert('end','#%02X%02X%02X'%(r,g,b))
    if 'win32' in sys.platform:
...
def _showDirs(event):
    ...
    if 'win32' in sys.platform:
        _toplevelf.wm_attributes('-disabled',1)
    _dir=tkFileDialog.askdirectory()
    if _dir:
...
def _showDir(event):
    ...
    if 'win32' in sys.platform:
        _toplevelf.wm_attributes('-disabled',1)
    file=tkFileDialog.askdirectory(title='Choose a directory')
    if file:
        event.widget.insert('end',file)
    if 'win32' in sys.platform:
        _toplevelf.wm_attributes('-disabled',0)  
...
def _checkRegexp(event):
    try:
        ...
        if m!=None and m.start()==0 and m.end()==len(text):
            ...
            #print 'No matches no'
            widget.delete(0,'end')
            tkMessageBox.showerror('Wrong input!','Validation error: the text "%s" doesn\'t match the expression "%s"' %(text,pat))
            widget.focus_set()
    except:
        pass
...
def getFormItemValue(*args):
    ...
    if len(args)!=2: 
        msg='''Numero incorrecto de argumentos para la funcion.
        La sintaxis correcta es _getFormItemValue(form,item_name)'''
        raise Exception(msg)
    form=args[0]
    if not form:
...
def setFormItemValue(*args):
    ...
    if len(args)!=3: 
        msg='''Numero incorrecto de argumentos para la funcion.
        La sintaxis correcta es _setFormItemValue(form,item_name,value)'''
        raise Exception(msg)
    form=args[0]
    if not form:
        raise Exception('Error: El formulario "%s" no esta definido'%args[0])
...
def getFormItem(*args):
    ...
    if len(args)!=2: 
        msg='''Numero incorrecto de argumentos para la funcion.
        La sintaxis correcta es _getFormItem(form,item_name)'''
        raise Exception(msg)
    form=args[0]
    if not form:
        raise Exception('Error: El formulario "%s" no esta definido'%args[0])
...
def callFormItem(*args):
    ...
    if len(args)!=4: 
        msg='''Numero incorrecto de argumentos para la funcion.
        La sintaxis correcta es _callFormItem(form,item_name,func_name,args_list)'''
        raise Exception(msg)
    form=args[0]
    if not form:
        raise Exception('Error: El formulario "%s" no esta definido'%args[0])
...
def setFormItemFont(*args):
    ...
    if len(args)!=3: 
        msg='''Numero incorrecto de argumentos para la funcion.
        La sintaxis correcta es _callFormItem(form,item_name,font_descriptor_id)'''
        raise Exception(msg)
    form=args[0]
    #print 'form: %s' % form
    if not form:
