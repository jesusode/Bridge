...
class TtkMultiListbox(ttk.Frame):
    def __init__(self, master,columns,lists,editable,callback,*opts,**kwopts ):
        ttk.Frame.__init__(self, master,*opts,**kwopts)
    ...
    def applyTags(self,item,tags,update=0):
        if update:
            ...
            if type(oldtags) in [type(''),type(u'')]:
    ...
    def __editCell(self,event):
        ...
        if self._rw:
            ...
            if self.colopts==None:
                tb=ttk.Entry(self,foreground='red')
                ...
                tb=ttk.Combobox(self,values=self.colopts[int(self._cl[1:])-1])
    ...
    def __filterCell(self,event):
        ...
        if len(self.tree.get_children())>1:
            ...
            if self._rw:
                ...
                tb=ttk.Combobox(self,values=uniques,state='readonly')
    ...
    def __doFilter(self,event):
        ...
        for item in self.tree.get_children():
            ...
            if type(texts[self.filterindex]) in [type(0),type(0L),type(0.0)]:
    ...
    def find(self,text,regex=0):
        ...
        for it in self.tree.get_children():
            ...
            for txt in texts:
                ...
                if type(txt) in [type(0),type(0L),type(0.0)]:
                    txt=str(txt)
                if regex==0:
                    if text in txt:
                        self.tree.selection_add(it)
